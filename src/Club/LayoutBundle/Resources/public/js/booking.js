    ...
    $.getJSON(url+'api/fields/'+location+"/"+getTime(date), function(json) {
        ...
        $.each(json.data.fields, function() {
            ...
            $.each(this.intervals, function() {
              ...
              $("#intervals").append('<div class="interval" id="interval_'+this.id+'" style="height: '+(field_height-5)+'px; top: '+top+'px; left: '+(day_diff*pixel_size)+'px; width: '+((diff*pixel_size)-1)+'px;">&#160;'+makeIntervalUrl(this.id,date,url)+'</div>');
              });
