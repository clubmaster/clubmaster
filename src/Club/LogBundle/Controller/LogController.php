...
class LogController extends Controller
{
    ...
    public function LogViewAction()
    {
        ...
        if (!($res = $this->canWriteToUploadPath()))
            $this->get('club_extra.flash')->addWarning(
                $this->get('translator')->trans('Cannot write to upload path <strong>%path%</strong>',array('%path%' => $this->container->getParameter('upload_path')))
            );

        if (($res = $this->installerFileExists()))
            $this->get('club_extra.flash')->addWarning(
                $this->get('translator')->trans('Remove installer file <strong>%path%</strong>', array('%path%' => $this->get('kernel')->getRootDir().'/installer'))
            );
